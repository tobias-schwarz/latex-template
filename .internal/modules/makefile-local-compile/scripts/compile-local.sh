...
if [[ "${found_latexmk}" -eq "0" ]]; then
    ...
    shell_escape=t latexmk -pdf -jobname="./${OUTPUT_DIR}/build" main.tex
elif [[ "${found_latex}" -eq "0" ]]; then
    echo -en "${GREEN}Using legacy latex..${RESET}\n"

    pdflatex --draftmode -recorder --jobname="./${OUTPUT_DIR}/build"  "main.tex"
    biber "${OUTPUT_DIR}/build"
    pdflatex --draftmode -recorder --jobname="./${OUTPUT_DIR}/build"  "main.tex"
    pdflatex -recorder --jobname="./${OUTPUT_DIR}/build"  "main.tex"
else
    echo -en "${LIGHT_RED}Could not find latex distribution${RESET}!\n"
    exit 1
fi

cp output/build.pdf main.pdf
