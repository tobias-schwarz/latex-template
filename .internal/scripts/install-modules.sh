...
# Cursor utility
function _res_lc() { PRINTED_LINES=0; }
function _inc_lc() { PRINTED_LINES=$((PRINTED_LINES + $1)); }
function _roll_lc() {
...
function requireGitConfig() {
  if ! git config "$1" &>>"${DHBW_BOOTSTRAP_LOG}"; then
    NEW_VALUE=""
...
function _installModule() {
  ...
  if [[ ! $2 == "force" ]]; then # If it isn't a forced installation, print module info
    new_module_info "${GREEN}" "${MODULE_NAME}" && _inc_lc 1
  ...
  install "$(pwd)" "$module_path" &>>"${DHBW_BOOTSTRAP_LOG}" || return 1
  commitChanges "${local_module_name}" "${module_description}" &>>"${DHBW_BOOTSTRAP_LOG}"
}
